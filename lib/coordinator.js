(function(module) {
    ...
    /**
     * Subscribe to queue information map updates
     *
     * Must be called only once in the constructor.
     *
     * @param {Function} callback Initial callback for "ready" event
     */
    ...
    /**
     * Increment shard size. Will retry automatically to keep size consistent.
     *
     * @param {Number} id Shard identifier
    ...
    Coordinator.prototype.incrementShardDataSize = function(id, size, callback) {
        if (!this.incrementors[id]) {
            this.incrementors[id] = new Incrementor(this.zk, "/shard-size-" + id);
        }

        this.incrementors[id].incrementSize(size, callback);
    };
    ...
    /**
     * Set shard "read-only" to disable write requests, useful for restoring.
     *
     * @param {Number} id Shard identifier
     * @param {Function} callback Callback to call after update
     */
    Coordinator.prototype.disableShard = function(id, callback) {
        var self = this;
